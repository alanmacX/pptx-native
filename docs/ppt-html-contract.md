...
- `docs/native-surface-inventory.md` — carrier/property matrix; check it before
  putting an effect or animation on an object type.
...
```
Declare what a group is on its container and the compiler expands it into native
timing rows. Known motifs: `timeline`, `layers`, `comparison`, `metricCluster`,
and `hubSpoke`. Motifs carry no visual style; they only choreograph roles such
as spine/node/card, left/right/center, or hub/spoke/satellite.
