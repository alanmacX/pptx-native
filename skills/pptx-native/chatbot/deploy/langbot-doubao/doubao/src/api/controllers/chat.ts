...
async function removeConversation(
  ...
  if (!convId) throw new Error("Cannot remove conversation without conversation_id");
  ...
  for (let attempt = 0; attempt < delays.length; attempt++) {
    ...
    try {
      // 删除接口会静默忽略缺少风控参数的请求，并返回空白 200。
      // 必须显式携带 msToken/a_bogus，并确认返回了 JSON data。
      const result = await request("post", "/samantha/thread/delete", refreshToken, {
        params: {
      ...
      if (result == null)
        throw new Error("Doubao returned an empty deletion response");
      logger.success(`Conversation ${convId} removed`);
      return;
    } catch (err) {
      lastError = err;
      logger.warn(`Conversation cleanup attempt ${attempt + 1}/${delays.length} failed`);
    }
  }
  throw new ConversationCleanupError(
    `Failed to remove conversation ${convId}`,
    { cause: lastError }
  );
}
...
async function createCompletion(
  ...
  return (async () => {
    ...
    // 删除成功后才向调用方返回，确保网页端不留下会话记录。
    await removeConversation(answer.id, refreshToken);

    return answer;
  })().catch((err) => {
    // 清理失败时禁止重新生成，否则会制造更多待删除会话。
    if (err instanceof ConversationCleanupError) throw err;
...
async function createCompletionStream(
  ...
  return (async () => {
    ...
    return createTransStream(response.data, async (convId: string) => {
      logger.success(
        `Stream has completed transfer ${util.timestamp() - streamStartTime}ms`
      );
      // 删除成功后才发送流式 [DONE]。
      await removeConversation(convId, refreshToken);
    });
  })().catch((err) => {
    if (retryCount < MAX_RETRY_COUNT) {
      logger.error(`Stream response error: ${err.stack}`);
...
function checkResult(result: AxiosResponse) {
  ...
  // 部分写操作（例如删除会话）成功时只有 code/msg，没有 data。
  if (code === 0) return data === undefined ? true : data;
...
function createTransStream(stream: any, endCallback?: Function) {
  ...
  const finish = () => {
    ...
    void (async () => {
      try {
        if (endCallback) await endCallback(convId);
      } catch (err) {
        logger.error(`Conversation cleanup failed before stream completion: ${err}`);
        if (!transStream.closed) transStream.destroy(err as Error);
        return;
      }
  ...
  const parser = createParser((event) => {
    try {
      if (event.type !== "event" || ended) return;
      // 解析JSON
      const rawResult = _.attempt(() => JSON.parse(event.data));
      if (_.isError(rawResult))
        throw new Error(`Stream response invalid: ${event.data}`);
